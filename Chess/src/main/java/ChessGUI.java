...
public class ChessGUI {
  ...
  private JButton firstClickButton;
  ...
  public final void initializeGui() {
    ...
    for (int ii = 0; ii < chessBoardSquares.length; ii++) {
      for (int jj = 0; jj < chessBoardSquares[ii].length; jj++) {
        final String column = COLUMNS.inverse().get(jj);
        final int row = chessBoardSquares[ii].length - ii;
        JButton b = new JButton();
        b.setMargin(buttonMargin);
        b.addActionListener(e -> makeMove(new Coordinate(column, row), b));
  ...
  private void makeMove(Coordinate coordinate, JButton button) {
    if (firstClick == null && coordinate != null) {
